...
function houseToMoney() {
    ...
    $('input[name="daikuan"]').on('change', function () {
        $('#lilv-rate_input').val($('[value="' + $(this).val() + '"]').text());
        $('#lilv-rate').val($(this).val());
        lilv = parseFloat($('#lilv-rate').val());
    });
    $('#fsq-rate').selectbox({
        change: function () {
            rate = parseFloat($('#fsq-rate').val());
        }
    });
    $('#fd-month').selectbox({
        change: function () {
            month = parseFloat($('#fd-month').val());
        }
    });
    $('#lilv-rate').selectbox({
        change: function () {
...
$('#zymbg').selectbox({
    change: function () {
        month = parseFloat($('#zymbg').val());
    }
});
$("#touzistart").click(function () {
    var gnsz = $("#gnsz").val().replace(/(^\s*)/g, "");
    var reg = /^[1-9]\d*$/;
    if (gnsz == "") {
        alert("购房总价不能为空");
        $("#gnsz").focus();
